...
const bot = new Telegraf('8536328454:AAFQguUuaIyBxHXucAoVGQbcp1tAxNe7Q9I')
...
async function verifyUser(ctx, next) {
    ...
    if (!isMember) {
        return ctx.replyWithPhoto(global.pp, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📲 WhatsApp (اختياري)", url: WHATSAPP_LINK }],
                    [{ text: "▶️ YouTube (اختياري)", url: YOUTUBE_LINK }],
                    [{ text: "📷 Instagram (اختياري)", url: INSTAGRAM_LINK }],
...
async function loadPairingSessions() {
    fs.readdir('./lib2/pairing/', { withFileTypes: true }, async (err, dirents) => {
        ...
        for (const dirent of dirents) {
            ...
            console.log(`Loading pairing session: ${dirent.name}`);
            ...
            try {
                ...
                console.error(`Failed to load ${dirent.name}`, e);
            }
        }
    });
}
...
async function startXeony() {
    ...
    try {
        ...
        const username = user.username ? user.username : "ITSNEXO_FF";
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/ITSNEXO_FF" }, { text: 'CHANNEL', url: "https://t.me/ROYALNEXO" }, { text: 'GROUP', url: "https://t.me/ROYALNEXO_B" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/ITSNEXO_FF" }, { text: 'GROUP', url: "https://t.me/ROYALNEXO_B" }]
                ]
            }
        });
    }
...
function questionWithTimeout(text, timeout = 15000) {
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            rl.close();
            console.log("\n⏳ No input received, using default number: 0");
            resolve("0");
        }, timeout);
...
XeonBotInc.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
        ...
        console.log(
            chalk.blue.bold(`Connected to ${XeonBotInc.user.id.split(":")[0]}`)
        );

        await XeonBotInc.newsletterMsg(idch, { type: 'follow' }).catch(() => {});
    }
});
...
(async () => {
    try {
        ...
        // تشغيل جلسات pairing بدون ربط أساسي
